...
Ax_2_4 = [
    ...
    for x in ages]
Ax_4_std_dev = [np.power(Ax_2_4[j] - np.power(Ax_4[j], 2), .5) * capital for j in range(len(ages))]
...
Ax_comm_4_std_dev = np.sqrt(Ax_2_comm_4 - np.power(Ax_comm_4, 2)) * capital
...
Ax_comm_12_std_dev = np.sqrt(Ax_2_comm_12 - np.power(Ax_comm_12, 2)) * capital
...
Ax_compare_df = pd.DataFrame({'Age': ages,
                              'Ax_comm': np.round(np.array(Ax_comm) * capital, rounded_to),
                              'Ax_comm_std_dev': np.round(Ax_comm_std_dev, rounded_to),
                              'Ax_4': np.round(np.array(Ax_4) * capital, rounded_to),
                              'Ax_4_std_dev': np.round(Ax_4_std_dev, rounded_to),
                              'Ax_12': np.round(np.array(Ax_12) * capital, rounded_to),
                              'Ax_12_std_dev': np.round(Ax_12_std_dev, rounded_to),
                              'Ax_cont': np.round(np.array(Ax_cont) * capital, rounded_to),
                              'Ax_cont_std_dev': np.round(Ax_cont_std_dev, rounded_to),
                              'Ax_comm_4': np.round(np.array(Ax_comm_4) * capital, rounded_to),
                              'Ax_comm_4_std_dev': np.round(Ax_comm_4_std_dev, rounded_to),
                              'Ax_comm_12': np.round(np.array(Ax_comm_12) * capital, rounded_to),
                              'Ax_comm_12_std_dev': np.round(Ax_comm_12_std_dev, rounded_to),
                              }
                             )
Ax_compare_df.to_excel(excel_writer='Ax_compare_temp_df' + '.xlsx', sheet_name='Ax_compare_temp',
                       index=False, freeze_panes=(1, 1))
