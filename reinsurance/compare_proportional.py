...
plt.axhline(y=qs.cedant_share, xmin=0.05, color='b', linestyle='--', label='Cedant share in Quota-share')
plt.axhline(y=qs.reinsurer_share, xmin=0.05, color='b', linestyle='--', label='Reinsurer share in Quota-share')
plt.xlabel('capital@risk')
plt.ylabel('share in %')
plt.title('Quota-Share versus Surplus')
plt.legend()
plt.show()
plt.savefig('compareQuotaShare_Surplus' + '.eps', format='eps', dpi=3600)